...
if ($method === 'GET') {
    // GET all curricular events with pagination and filters
    ...
    try {
        ...
        $count_query = "SELECT COUNT(*) as total FROM events WHERE event_type = 'Curricular'";
        $query = "SELECT e.*, CONCAT(u.first_name, ' ', u.last_name) as organizer_name
                  FROM events e
                  LEFT JOIN users u ON e.created_by = u.id
                  WHERE e.event_type = 'Curricular'";
    ...
    try {
        ...
        $query = "INSERT INTO events (title, description, event_type, event_date, event_time, venue, organizer, created_by, created_at) 
                  VALUES (:title, :description, 'Curricular', :event_date, :event_time, :venue, :organizer, :created_by, NOW())";
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Curricular event created successfully',
            'data' => [
                'id' => $event_id,
                'title' => $data->title,
