...
try {
    ...
    // Check if faculty has schedules
    $check_schedules = "SELECT COUNT(*) as count FROM schedules WHERE faculty_id = :faculty_id";
    $check_stmt = $db->prepare($check_schedules);
    $check_stmt->bindParam(':faculty_id', $faculty_id);
    $check_stmt->execute();
    $schedule_count = $check_stmt->fetch(PDO::FETCH_ASSOC)['count'];
    ...
    if ($schedule_count > 0) {
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Faculty member has been deactivated due to existing schedules',
            'deactivated' => true
        ]);
    } else {
        ...
        $delete_stmt = $db->prepare($delete_faculty);
        $delete_stmt->bindParam(':faculty_id', $faculty_id);
        $delete_stmt->execute();
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Failed to delete faculty: ' . $e->getMessage()
    ]);
}
?>